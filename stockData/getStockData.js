...
export const currentPrice = async (stock) => {
  try {
    var res = await fetch(
      `ec2-34-230-21-175.compute-1.amazonaws.com/financeratio?symbol=${stock}`
    );
    // return res.json();
    if (res.ok) {
      return res.json();
...
export const pastData = async (stock, date, endDate) => {
  try {
    var res = await fetch(
      `ec2-34-230-21-175.compute-1.amazonaws.com/pastdata?symbol=${stock}&start=${date}&end=${endDate}`
    );
    if (res.ok) {
      return res.json();
    } else {
